...
struct Dinitz{
    ...
    // Agrega arista(st -> en) consu capacidad
...
signed main()
{
    ...
    fore(i, 0, m)
    {
        ...
        // cout<<n + i<<' '<<b<<'\n';
        // cout<<s<<' '<<n + i<<'\n';
        dinic.addEdge(n + i, a, 1);
        dinic.addEdge(n + i, b, 1);
        dinic.addEdge(s, n + i, 1);
    }
    int flo = 0;
    int res = 0;
    while(flo < m)
    ...
    fore(i, 0, m)
    {
        if(dinic.G[n + i][0].f == dinic.G[n + i][0].cap)
            cout<<edges[i].f<<' '<<edges[i].s<<'\n';
        else
            cout<<edges[i].s<<' '<<edges[i].f<<'\n';
    }
	return 0;
}
// Se vuelve más fácil,
