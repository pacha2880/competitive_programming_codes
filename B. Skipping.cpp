...
signed main()
{
    ...
    while(t--)
    {
        ...
        {
            pref += ar[i];
            while(!llega.empty() && llega.begin()->f < i)
            {
                precio.erase({dp[llega.begin()->s], llega.begin()->s}),
                llega.erase(llega.begin());
            }
            if(!precio.empty())
            {
                // cout<<pref<<' '<<precio.begin()->f<<'\n';
                dp[i] = precio.begin()->f + ar[i];
                res = max(res, pref - precio.begin()->f);
                // cout<<' '<<dp[i]<<' '<<er[i]<<'\n';
                precio.insert({dp[i], i}), llega.insert({er[i], i});
            }
        }
        cout<<res<<'\n';
    }
