...
template <uint64_t Mod = 1'000'003> 
struct Modular {
    using M = Modular;
    uint64_t v;
...
namespace ntt {
    ...
    template <uint64_t Mod> void ntt(vector<Modular<Mod>>& a, bool inverse) {
    ...
    */
    vector<Mint> operator*(const vector<Mint>& l, const vector<Mint>& r) {
        ...
        for (int i = 0; i < n + m - 1; ++i) {
            uint64_t y0 = l0[i].v;
            uint64_t y1 = (im0 * (l1[i] - y0)).v;
            uint64_t y2 = (im0m1 * (l2[i] - y0) - im1 * y1).v;
...
inline void dfs(ll u, ll pp){
    vector<Mint> ret(top, Mint(1));
    for(auto xd: adj[u]) if(xd != pp){
        dfs(xd, u);
        vector<Mint> acum(top, Mint(0));
        Mint sum = 0;
        FER(i, 0, top) sum += dp[xd][i], acum[i] = sum;
        FER(i, 1, top) ret[i] *= acum[i];
    }
    FER(i, 1, top) dp[u][i] = ret[i];
}
...
int main(){
    ...
    auto ans = interpolate(x, y);
    Mint BunChan = 0, coe = Mint(1);
    for(auto xd: ans.a) {
        BunChan += coe * xd;
        coe *= Mint(d);
    }
