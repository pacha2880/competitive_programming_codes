...
const int MAX_SIZE = 200000;  // Sufficient for most competitive programming constraints
int globalStack[MAX_SIZE];    // Fixed-size array to use as a stack for indices
int top = -1;                 // Stack pointer initialized to -1

// Function to compute the maximum area in a histogram
int maxHistogramArea(const vector<int>& heights) {
    int maxArea = 0;
    int n = heights.size();

    for (int i = 0; i < n; ++i) {
        while (top >= 0 && heights[globalStack[top]] > heights[i]) {
            int h = heights[globalStack[top--]];  // Pop the top element
            int width = (top == -1) ? i : i - globalStack[top] - 1;
            maxArea = max(maxArea, h * width);
        }
        globalStack[++top] = i;  // Push current index to the stack
    }

    // Calculate area for remaining elements in stack
    while (top >= 0) {
        int h = heights[globalStack[top--]];  // Pop the top element
        int width = (top == -1) ? n : n - globalStack[top] - 1;
        maxArea = max(maxArea, h * width);
    }
...
signed main()
{
    ...
    fore(i, 0, n)
        cin>>ar[i];
    int res = maxHistogramArea(ar);
    fore(i, 0, n)
    {
        if(x > ar[i])
        {
            swap(x, ar[i]);
            res = max(res, maxHistogramArea(ar));
            swap(x, ar[i]);
        }
    }
    cout<<res<<'\n';
