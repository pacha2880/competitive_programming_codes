...
int main()
{
    ...
    int n, k, ms, me;
    cin>>n>>k>>ms>>me;
    vector<vector<double>> A(2 * (n - k + 1) + n, vector<double>(n));
    vector<double> b(2 * (n - k + 1) + n), c(n);
    fore(i, 0, n)
    {
        cin>>sl[i];
        c[i] += sl[i];
    ...
    auto cat = simplex(A, b, c);
    string s;
    ll su = 0;
    fore(i, 0, n) 
    {
        su += cat.s[i] > EPS ? sl[i] : ea[i];
        s += cat.s[i] > EPS ? 'S' : 'E';
    }
    cout<<su<<'\n'<<s<<'\n';
    return 0;
