...
signed main()
{
    ...
    while(t--)
    {
        ...
        vector<vector<vi>> res(n, vector<vi>(n, vi(n, MOD)));
        fore(i, 0, n) fore(j, 0, n) res[i][j][0] = 0;
        fore(i, 0, n)
        {
            queue<int> que;
            que.push(i);
            ...
            while(!que.empty())
            {
                ...
                for(ii cat : g[node])
                {
                    int x = cat.f, pes = cat.s;
                    if(!vis[x])
                        que.push(x), vis[x] = 1;
                    res[i][x][1] = min(res[i][x][1], pes);
                    fore(j, 2, n)
                        res[i][x][j] = min(res[i][x][j], res[i][node][j - 1]);
                }
            }
            cout<<"# "<<i + 1<<'\n';
            fore(j, 0, n)
            {
                    cout<<"$ "<<j + 1<<'\n';
                    fore(k, 0, n) cout<<res[i][j][k]<<' ';
                    cout<<'\n';
            }
            fore(j, 0, n)
                fore(k, 1, n)
                    res[i][j][k + 1] = min(res[i][j][k + 1], res[i][j][k]);
        }
        cout<<"paparapa\n";
        while(q--)
        {
