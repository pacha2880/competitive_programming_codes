...
{
    pi[node] = ins;
    in[ar[node].f] = min(in[ar[node].f], ins);
    for(int x : g[node])
        if(x != par)
            ins++, dfs(x, node);
    out[ar[node].f] = max(out[ar[node].f], ins);
    put[node] = ins;
}
signed main()
{
    ...
    while(t--)
    {
        ...
        fore(i, 0, n + 1) in[i] = MOD, out[i] = -MOD;
        ins = 0;
        dfs(0);
        // cout<<"asdf\n";
        int ini = MOD, outo = -MOD;
        int uni = MOD, uout = -MOD;
        ...
        for(int i = n - 2; i > -1; i--)
        {
            ini = min(ini, in[ar[i + 1].f]);
            outo = max(outo, out[ar[i + 1].f]);
            if(ar[i].f != ar[i + 1].f)
                uni = ini, uout = outo;
            // cout<<ar[i].f<<' '<<uni<<' '<<uout<<'\n';
            // cout<<ar[i].s<<'\n';
            // cout<<ar[i].f<<' '<<ar[i + 1].f<<'\n';
            // cout<<pi[ar[i].s]<<' '<<put[ar[i].s]<<'\n';
            if(pi[ar[i].s] > uni || put[ar[i].s] < uout)
            {
                res = ar[i].s + 1;
                break;
            }
