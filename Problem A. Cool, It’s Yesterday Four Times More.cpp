...
{
    if(x == -1 || y == -1 || x == n || y == m || tab[x][y] == 'O' || vis[x][y]) return;
    bi[traba(x, y)][po] = 0;
    mayo = max(mayo, traba(x, y));
    mino = min(mino, traba(x, y));
    vis[x][y] = 1;
    fore(i, 0, 4)
