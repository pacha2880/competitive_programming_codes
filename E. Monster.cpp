...
signed main()
{
    ...
    while(t--)
    {
        ...
        auto f = [&](int time)
        {
            int cuantos = time / (k + 1);
            int costo = cuantos * k * x + cuantos * y;
            int modo = time % (k + 1);
            costo += modo * x;
            int ataque = cuantos * k + modo;
            int vida = z - k * cuantos * (cuantos + 1) / 2;
            if(vida > 0)
                costo += (vida + ataque - 1) / ataque * y;
            return costo;
        };
        auto f1 = [&](int time)
        {
            ...
            int ataque = cuantos * k + modo;
            int vida = z - k * cuantos * (cuantos + 1) / 2;
            if(vida < 0) return 0ll;
            return (vida + ataque - 1) / ataque;
        };
        int res = MOD * MOD;
        fore(i, 1, min(50010ll, 2 * z + 2))
            res = min(res, f(i));
        fore(i, 1, 30010)
        {
            int lo = 1, hi = 2 * z + 2, mid;
            int resa = 1;
            while(lo < hi)
            {
                mid = (lo + hi) / 2;
