...
signed main()
{
    ...
    while(t--)
    {
        ...
        fore(i, 0, n)
        {
            while(!popo.empty() && -popo.back().f <= ar[i])
                popo.pop_back();
            popo.pb({-ar[i], i});
            auto cat = lower_bound(all(popo), mp(-er[i], 0ll));
            if(cat != popo.end() && cat->f == -er[i] && query(0, n - 1, 0, cat->s, i) >= er[i])
                res[i] = 1;
        }
        popo.clear();
        for(int i = n - 1; i > -1; i--)
        {
            ...
            popo.pb({-ar[i], i});
            auto cat = lower_bound(all(popo), mp(-er[i], 0ll));
            if(cat != popo.end() && cat->f == -er[i] && query(0, n - 1, 0, i, cat->s) >= er[i])
                res[i] = 1;
        }
        bool bo = 1;
        fore(i, 0, n) bo &= res[i];
