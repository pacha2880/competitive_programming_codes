...
struct node
{
    int maxi, sum, maxl, maxr;
    node() {maxi = sum = maxl = maxr = 0;}
    node(int val) {maxi = sum = maxl = maxr = val;}
};
node join(node a, node b)
{
	node res;
	res.maxi = max(max(a.maxi, b.maxi), a.maxr + b.maxl);
	res.sum = a.sum + b.sum;
	res.maxl = max(a.maxl, a.sum + b.maxl);
	res.maxr = max(a.maxr + b.sum, b.maxr);
	return res;
}
int ar[100000];
struct st
...
int main()
{
  ...
  node ras;
	...
	while(m--)
	{
        ...
        {
            cout<<query(t, 0, n - 1, a - 1, b - 1).maxi<<'\n';
        }
	}
	return 0;
}
