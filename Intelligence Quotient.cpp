...
// Anñade arista (st -> en) con su capacidad
...
int nodes, S, T; // asignar estos valores al armar el grafo
				// nodes = nodos en red de flujo, Hacer G.clear(), G.resize(node);
...
signed main()
{
	...
	fore(i, 0, n)
		fore(j, 0, m)
			if(!beta[i][j])
				addEdge(i, j + n, MOD * MOD);
	int flo = maxFlow();
	set<int> mato, compo;
	fore(i, 1, n + 1) mato.insert(i);
	fore(i, 1, m + 1) compo.insert(i);
	dfs(S);
	for(auto cat : G[S])
		if(!vis[cat.to] && cat.f == cat.cap)
			mato.erase(cat.to + 1);
	fore(i, n, n + m)
		if(vis[i])
			for(auto cat : G[i])
				if(cat.to == T && cat.f == cat.cap)
					compo.erase(i - n + 1);
	cout<<sasa - flo<<'\n';
	cout<<mato.size()<<'\n';
	if(sz(mato))
