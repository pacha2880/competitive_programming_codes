...
signed main()
{
    ...
    while(t--)
    {
        ...
        // for(ii cat : ma) cout<<'$'<<cat.f<<' '<<cat.s<<'\n';
        // for(ii cat : me) cout<<'#'<<cat.f<<' '<<cat.s<<'\n';
        if(sz(ma) == n / 2 && ma == me)
        {
            vi po(n + 1);
            fore(i, 0, n) po[ar[i]] = i;
            vii res;
            int con = 0;
            for(ii cat : ma)
            {
                if(po[cat.f] != con)
                {
                    res.pb({con + 1, po[cat.f] + 1});
                    ar[po[cat.f]] = ar[con];
                    po[ar[con]] = po[cat.f];
                }
                if(po[cat.s] != n - 1 - con)
                {
                    res.pb({n - con, po[cat.s] + 1});
                    ar[po[cat.s]] = ar[n - 1 - con];
                    po[ar[n - 1 - con]] = po[cat.s];
                }
                con++;
            }
            cout<<sz(res)<<'\n';
