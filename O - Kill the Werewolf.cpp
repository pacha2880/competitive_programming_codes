...
signed main()
{
    ...
    cin>>n;
    vii cho(n);
    fore(i, 0, n) cin>>cho[i].f>>cho[i].s, cho[i].f--, cho[i].s--;
    int res = 0;
    S = 2 * n, T = 2 * n + 1;
    nodes = 2 * n + 2;
    fore(i, 0, n)
    {
        ...
        fore(j, 0, n)
        {
            if(j == cho[i].f || j == cho[i].s)
                addEdge(j + n, T, in - 2);
            else
                addEdge(j + n, T, in - 1);
            addEdge(S, j, 1);
            if(j != i && cho[j].f != i && cho[j].s != i)
                addEdge(j, cho[j].f + n, 1),
                addEdge(j, cho[j].s + n, 1);
        }
        int flow = maxFlow();
        // cout<<in<<'\n';
        // cout<<'#'<<i<<' '<<flow<<'\n';
