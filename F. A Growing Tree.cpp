    ...
    while(t--)
    {
        ...
        fore(i, 0, q)
        {
            ...
            if(sz(que[i]) == 1)
            {
                ...
                update(0, n - 1, 0, ini[que[i][0]], ini[que[i][0]], 
                -query(0, n - 1, 0, ini[que[i][0]], ini[que[i][0]]));
            }
            else
                update(0, n - 1, 0, ini[que[i][1]], fini[que[i][1]], que[i][2]);
        }
        fore(i, 0, n)
            cout<<query(0, n - 1, 0, ini[i], ini[i])<<' ', g[i].clear();
        cout<<'\n';
    }
	return 0;
}
