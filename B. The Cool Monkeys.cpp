...
signed main()
{
    ...
    {
        int to = n + m;
        Dinitz dinic(2 * to + 2, 2 * to, 2 * to + 1);
        fore(i, 0, k)
            dinic.addEdge(2 * to, n - 1 - i, 1), dinic.addEdge(to + n + i, 2 * to + 1, 1);
        fore(i, 0, n)
            dinic.addEdge(i, to + i, 1);
        fore(i, 0, m)
            dinic.addEdge(i + n, to + i + n, 1);
        fore(i, 0, n)
        {
            fore(j, 0, m)
            {
                ...
                {
                    // cout<<i<<' '<<j<<'\n';
                    dinic.addEdge(to + i, j + n, 1);
                    dinic.addEdge(to + j + n, i, 1);
                }
            }
        }
        int flo = dinic.maxFlow();
        // cout<<flo<<'\n';
        if(flo == k)
            bo = true;
        swap(n, m);
