...
signed main()
{
	...
	int n;
    cin>>n;
    vector<vi> pro(n, vi(3));
    set<int> st;
    fore(i, 0, n) 
    {
        fore(j, 0, 3) cin>>pro[i][j];
        st.insert(pro[i][0]);
        st.insert(pro[i][1]);
    }
    int con = 0;
    ...
    con++;
    fore(i, 0, n) fore(j, 0, 2) pro[i][j] = ma[pro[i][j]];
    sort(all(pro));
    vi dp(con + 1);
    int ultac = con - 1;
    for(int i = n - 1; i > -1; i--)
    {
        while(ultac > pro[i][0])
            ultac--, dp[ultac] = max(dp[ultac], dp[ultac + 1]);
        dp[pro[i][0]] = max(dp[pro[i][0]], dp[pro[i][1] + 1] + pro[i][2]);
    }
    cout<<dp[0]<<'\n';
	return 0;
}
