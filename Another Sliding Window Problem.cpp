...
signed main()
{
    ...
    while(q--)
    {
        ...
        {
            li = max(li, i);
            while(li > i && ar[i] + ar[li] > x)
                li--;
            peruanos[i] = li;
        }
        for(int i = n - 1; i > 0; i--)
            peruanos[i - 1] = min(peruanos[i] + 1, peruanos[i - 1]);
        int res = 0;
        fore(i, 0, n)
        {
            if((peruanos[i] - i) % 2 == 1 && i < n && ar[peruanos[i] + 1] <= x)
                peruanos[i]++;
            peruanos[i] = min(peruanos[i], n - 1);
            // cout<<i<<' '<<peruanos[i]<<'\n';
            // cout<<sum(i, peruanos[i])<<'\n';
            // cout<<ar[i] * (peruanos[i] - i + 1)<<'\n';
            res += sum(i, peruanos[i]) - ar[i] * (peruanos[i] - i + 1);
        }
        cout<<res<<'\n';
    }
	return 0;
