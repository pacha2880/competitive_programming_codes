...
int dp[5][1010], vis[5][1010];
string ar[1010];
int n;
string s = "narek";
vi es(500);
int f(int let, int pos)
{
    if(pos == n) return -2 * let;
    if(vis[let][pos]) return dp[let][pos];
    vis[let][pos] = 1;
    dp[let][pos] = 0;
    int cur = let;
    for(char ch : ar[pos])
    ...
    dp[let][pos] = max(f(let, pos + 1), dp[let][pos] + f(cur, pos + 1));
    // cout<<s[let]<<' '<<pos<<' '<<' '<<dp[let][pos]<<'\n';
    return dp[let][pos];
}
signed main()
{
    ...
    while(t--)
    {
        int  m;
        cin>>n>>m;
        fore(i, 0, 5) fore(j, 0, n) vis[i][j] = 0;
        fore(i, 0, n) cin>>ar[i];
        cout<<f(0, 0)<<'\n';
    }
	return 0;
