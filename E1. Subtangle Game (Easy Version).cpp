...
signed main()
{
    ...
    while(t--)
    {
        ...
        fore(i, 0, l) fore(j, 0, n) fore(k, 0, m) dp[i][j][k] = 0;
        for(int i = n - 1; i > -1; i--)
            for(int j = m - 1; j > -1; j--)
            {
                fore(k, 0, l - 1)
                {
                    if(i < n - 1 && dp[k][i + 1][j])
                        dp[k][i][j] = 1;
                    else if(j < m - 1 && dp[k][i][j + 1])
                        dp[k][i][j] = 1;
                    else if(i < n - 1 && j < m - 1 && tab[i][j] == ar[k] && !dp[k + 1][i + 1][j + 1])
                        dp[k][i][j] = 1;
                    else if((i == n - 1 || j == m - 1) && tab[i][j] == ar[k])
                        dp[k][i][j] = 1;
                    // cout<<k<<' '<<i<<' '<<j<<' '<<dp[k][i][j]<<'\n';
                }
                if(i < n - 1 && dp[l - 1][i + 1][j])
                    dp[l - 1][i][j] = 1;
                else if(j < m - 1 && dp[l - 1][i][j + 1])
                    dp[l - 1][i][j] = 1;
                else if(tab[i][j] == ar[l - 1])
                    dp[l - 1][i][j] = 1;
                // cout<<l-1<<' '<<i<<' '<<j<<' '<<dp[l-1][i][j]<<'\n';
            }
        if(dp[0][0][0])
            cout<<"T\n";
        else
