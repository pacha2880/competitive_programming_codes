...
signed main()
{
    ...
    fore(cas, 1, t + 1)
    {
        ...
        vi pingo(n), sali(n);
        int pingos = 0;
        fore(i, 0, n) cin>>pozo[i].f>>pozo[i].s>>pingo[i]>>sali[i], pingos += pingo[i];
        fore(i, 0, n) fore(j, i + 1, n)
        {
            if(hypot(pozo[i].f - pozo[j].f, pozo[i].s - pozo[j].s) < madis + EPS)
                g[i].pb(j), g[j].pb(i);
        }
        vi res;
        fore(i, 0, n)
        {
            ...
            dinic.addEdge(i, dinic.T, MOD);
            fore(i, 0, n)
            {
                dinic.addEdge(i, i + n, sali[i]);
                dinic.addEdge(dinic.S, i, pingo[i]);
                for(int x : g[i])
                    dinic.addEdge(i + n, x, MOD);
            }
            if(dinic.maxFlow() == pingos)
                res.pb(i);
        }
        cout<<"Case "<<cas<<':';
        if(res.empty())
