...
signed main()
{
    ...
    while(t--)
    {
        ...
        vi ar(n);
        fore(i, 0, n) cin>>ar[i];
        int mai = *max_element(all(ar));
        int lo = 1, hi = mai + k / n, mid, res = 1;
        while(lo <= hi)
        {
            mid = (lo + hi) / 2;
            int coso = 0;
            fore(i, 0, n)
                coso += max(0ll, mid - ar[i]);
            if(coso <= k)
                res = mid, lo = mid + 1;
            else
                hi = mid - 1;
        ...
        fore(i, 0, n)
        {
            if(ar[i] < res) k -= res - ar[i], ar[i] = res;
        }
        // RAYA;
        ...
        fore(i, 0, n)
        {
            if(ar[i] == res && k) ar[i]++, k--;
        }
        int cana = 0;
        fore(i, 0, n) cana += ar[i] > res;
        cout<<(res - 1) * n + 1 + cana<<'\n';
    }
	return 0;
}
