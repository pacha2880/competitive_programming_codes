...
ii join(ii a, ii b)
{
    return {a.f + max(0ll, b.f - a.s), a.s + b.s};
}
void init(int b, int e, int node)
{
    ...
    init(b, mid, l);
    init(mid + 1, e, r);
    t[node] = {t[l].f + max(0ll, t[r].f - t[l].s), t[l].s + t[r].s};
}
...
int 
void sorto(int b, int e)
{
    if(b < e)
    {
        int mid = (b + e) / 2;
        sorto(b, mid); sorto(mid + 1, b);
    }
}
signed main()
{
    ...
    while(t--)
    {
        int n;
        cin>>n;
        int mimi = MOD, sa = 0, sb = 0;
        int su = 0;
        fore(i, 0, n) cin>>ar[i].f>>ar[i].s, su += ar[i].f;
        init(0, n - 1, 0);
        sorto(0, n - 1);
        cout<<t[0].f + su<<'\n';
    }
