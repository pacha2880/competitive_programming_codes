...
signed main()
{
    ...
    vector<vi> dis(k, vi(n, MOD * MOD));
    vector<vi> vis(k, vi(n));
    dis[0][0] = 0;
    que.push({0, 0, 0});
    while(!que.empty())
    {
        int node = que.top()[2], nivel = que.top()[1];
        que.pop();
        if(vis[nivel][node]) continue;
        vis[nivel][node] = 1;
        int d = dis[nivel][node];
        // cout<<node<<' '<<nivel<<' '<<d<<'\n';
        for(ii x : g[node])
        {
            int du = d + x.s;
