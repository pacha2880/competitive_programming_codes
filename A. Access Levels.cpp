...
signed main()
{
    ...
    fore(i, 0, m)
    {
        ...
        addEdge(i + m, T, 1);
        if(docs[i].count() == 0)
            cols[i] = 1, vals[i] = 1e9, gatos++, esgato[i] = 1;
        else
        fore(j, i + 1, m)
        {
            // cout<<"%%%\n";
            // fore(k, 0, n) cout<<docs[i][k];
            // cout<<'\n';
            // fore(k, 0, n) cout<<docs[j][k];
            // cout<<'\n';
            if(!docs[j].count()) continue;
            if((docs[i] & docs[j]) == docs[j])
            {
                // cout<<'^'<<i<<' '<<j<<'\n';
                addEdge(i, j + m, 1);
            }
            else if((docs[i] & docs[j]) == docs[i])
            {
                // cout<<'*'<<j<<' '<<i<<'\n';
                addEdge(j, i + m, 1);
            }
    ...
    int flo = maxFlow();
    // cout<<flo<<'\n';
    int resusito = m - flo - gatos;
    vi par(m, -1), hasch(m);
    fore(i, 0, m)
        for(auto x : G[i])
            if(x.to != S && x.cap == x.f)
                par[x.to - m] = i, hasch[i] = 1;
    if(resusito == 0)
    resusito = 1;
    cout<<resusito<<'\n';
    vector<vi> res(n, vi(resusito, 1));
    int pupi = 0;
    fore(i, 0, m)
    {
        if(!hasch[i] && !esgato[i])
    ...
    fore(i, 0, m) cout<<cols[i]<<' ';
    cout<<'\n';
    fore(i, 0, m) cout<<vals[i]<<' ';
    cout<<'\n';
    fore(i, 0, n)
    {
        fore(j, 0, resusito)
