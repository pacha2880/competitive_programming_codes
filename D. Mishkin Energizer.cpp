...
signed main()
{
    ...
    while(t--)
    {
        ...
        fore(i, 0, 2 * n)
        {
            while(ch == que.front() || con[3 - po[ch] - po[que.front()]] == n) {
                while(ch == que.front())
                    ch = que.front(), que.pop_front(), pos++;
                while(con[3 - po[ch] - po[que.front()]] == n)
                    ch = que.front(), que.pop_front(), pos++;
            }
            cout<<pos<<'\n';
            int let = 3 - po[ch] - po[que.front()];
            con[let]++;
            que.push_front(can[let]);
        }
    }
}
// Se vuelve más fácil,
