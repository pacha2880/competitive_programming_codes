...
signed main()
{
    ...
    while(t--)
    {
        ...
        cin>>l>>n>>m;
        vi ar(l);
        fore(i, 0, l) cin>>ar[i], ar[i]--;
        vector<vii> aruba(n * m);
        fore(i, 0, n)
            fore(j, 0, m)
            {
                int x;
        ...
        fore(i, 0, n * m)
        {
            ...
            for(ii x : aruba[i])
            {
                while(!neo.empty() && neo.back().s <= x.s) neo.pop_back();
                neo.pb(x);
            }
            swap(neo, aruba[i]);
        ...
        for(int i = l - 1; i > -1; i--)
        {
            ...
            for(ii x : aruba[ar[i]])
            {
                while(nico < sz(biborita) && biborita[nico].f <= x.f)
                    nico++;
                if(nico == sz(biborita))
                    neo.pb(x);
                else if(biborita[nico].s <= x.s)
                    neo.pb(x);
            }
            swap(neo, biborita);
            // for(ii x : biborita)
