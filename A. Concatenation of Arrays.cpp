...
signed main()
{
    ...
    while(t--)
    {
        ...
        ordered_set<ii> st;
        fore(i, 0, n)
            cin>>ar[i].f>>ar[i].s, st.insert({ar[i].f, ma[ar[i].f]++}), st.insert({ar[i].s, ma[ar[i].s]++});
        vector<vi> er(n);
        fore(i, 0, n)
        {
            er[i] = vi({st.order_of_key({ar[i].f, 0}) + st.order_of_key({ar[i].s, 0}) - (ar[i].f > ar[i].s), ar[i].f, ar[i].s});
        }
        sort(all(er));
        fore(i, 0, n)
            cout<<er[i][1]<<' '<<er[i][2]<<' ';
