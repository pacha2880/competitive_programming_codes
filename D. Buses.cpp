...
int main()
{
	...
	st.insert(0);
	st.insert(n);
	fore(i, 0, m) {cin>>ari[i].s>>ari[i].f; st.insert(ari[i].s); st.insert(ari[i].f);}
	n = st.size();
	map<int, int> ma;
	int co = 0;
	for(int x : st)
	...
	fore(i, 0, m)
	{
		// cout<<ma[ari[i].s]<<' '<<ma[ari[i].f]<<'\n';
		au = query(0, n - 1, 0, ma[ari[i].s], ma[ari[i].f] - 1);
		update(0, n - 1, 0, ma[ari[i].f], au);
	}
	// cout<<n<<'\n';
