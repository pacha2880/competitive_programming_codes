...
signed main()
{
    ...
    dfs();
    fore(i, 0, n)
        update(0, n, 0, ini[i], ar[i]), update(0, n, 0, fini[i], -ar[i]);
    while(q--)
    {
        int a, b;
        cin>>a>>b;
        if(a == 2){
            cout<<query(0, n, 0, 0, ini[b - 1])<<'\n';
        }
        else{
            int c;
            cin>>c;
            b--;
            update(0, n, 0, ini[b], c - ar[b]), update(0, n, 0, fini[b], ar[b] - c);
            ar[b] = c;
        }
    }
	return 0;
