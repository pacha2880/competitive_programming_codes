...
signed main()
{
    ...
    fore(i, 0, t)
    {
        ...
        int dif = b - a;
        gp_hash_table<int, vi, my_hash> difu;
        for(int j = 1; j * j <= dif; j++)
        {
            if(dif % j == 0)
            {
                ...
                if(j * j != dif)
                {
                    ...
                    fact(dif / j, mama);
                    for(auto cat : mama)
                        difu[dif / j].pb(cat.f), su += cat.s;
                }
            }
        }
        int res;
        priority_queue<ii> que;
        gp_hash_table<int,int,my_hash> dis;
        dis[a | (b<<32)] = 0;
        que.push({0, a | (b<<32)});
        while(true)
        {
            a = que.top().s<<32>>32, b = que.top().s>>32, pas = -que.top().f;\
            que.pop();
            dif = b - a;
            // cout<<a<<' '<<b<<' '<<pas<<' '<<dif<<'\n';
            if(a == 1)
            ...
            {
                // cout<<x<<'\n';
                    aa = a / x, bb = b / x, ha = aa | (bb<<32);
                if(a % x == 0)
                {
                    if(dis.find(ha)==dis.end() || dis[ha] > pas + 1)
                    {
                        dis[ha] = pas + 1;
                        que.push({-pas - 1, ha});
                    }
                ...
                else
                {
                    ...
                    mo = a % x;
                    if(dis.find(ha)==dis.end() || dis[ha] > pas + mo + 1)
                    {
                        dis[ha] = pas + mo+ 1;
                        que.push({-pas -mo - 1, ha});
                    }
                    aa++, bb++;
                    ha = aa | (bb<<32);
                    if(dis.find(ha)==dis.end() || dis[ha] > pas + x -mo + 1)
                    {
                        dis[ha] = pas + x - mo + 1;
                    que.push({-pas -(x - mo) - 1, ha});
                    }
                }
            }
            aa = 1, bb = b - a + 1, ha = aa | (bb<<32);
            if(dis.find(ha)==dis.end() || dis[ha] > pas + a - 1)
            {
                dis[ha] = pas + a - 1;
                que.push({-pas -a + 1, ha});
            }
