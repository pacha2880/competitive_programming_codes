...
signed main()
{
    ...
    Dinitz dinic(n * m + 4, n * m + 2, n * m + 3);
    int dx[] = {0, 0, 1, -1}, dy[] = {1, -1, 0, 0};
    dinic.addEdge(n * m + 1, n * m, MOD);
    fore(i, 0, n)
        fore(j, 0, m)
        {
            ...
            if((i + j) & 1)
            {
                if(tab[i][j] == '2')
                {
                    dinic.addEdge(n * m, n * m + 3, 1);
                    dinic.addEdge(n * m + 2, x, 1);
                }
                else
                    dinic.addEdge(n * m, x, 1);
                fore(k, 0, 4)
                {
                    int ii = i + dx[k], jj = j + dy[k];
                    if(ii >= 0 && ii < n && jj >= 0 && jj < m && tab[ii][jj] != '1' && (tab[i][j] == '2' || tab[ii][jj] == '2'))
                        dinic.addEdge(x, ii * m + jj, 1);
                }
            }
            else
            {
                if(tab[i][j] == '2')
                {
                    dinic.addEdge(x, n * m + 3, 1);
                    dinic.addEdge(n * m + 2, n * m + 1, 1);
                }
                else
                    dinic.addEdge(x, n * m + 1, 1);
            }

        }
    dinic.maxFlow();
    bool bo = true;
    for(auto cat : dinic.G[n * m + 2])
        if(cat.cap != cat.f && cat.cap == 1)
            bo = false;
    fore(i, 0, n * m + 1)
        for(auto cat : dinic.G[i])
            if(cat.cap != cat.f && cat.cap == 1 && cat.to == n * m + 3)
                bo = false;
