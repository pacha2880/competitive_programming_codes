...
signed main()
{
    ...
    while(t--)
    {
        ...
        int suzume = 0;
        vi le(n), ri(n);
        fore(i, 0, n) cin>>le[i], suzume += le[i];
        fore(i, 0, n) cin>>ri[i], suzume += ri[i];
        vi mimi(n);
        fore(i, 0, n) mimi[i] = min(le[i], ri[i]);
        sort(all(mimi));
        int suzi = 0;
        fore(i, 0, n - k + 1) suzi += mimi[i];
        cout<<suzume - suzi + 1<<'\n';
        continue;
        sort(all(le)); sort(all(ri));
        int lei = 0, rei = 0;
        vi caca;
        fore(i, 0, n - k + 1) caca.pb(le[i]), caca.pb(ri[i]);
        sort(all(caca));
        fore(i, 0, n - k )
            lei += le[i], rei += ri[i];
        cout<<lei<<' '<<rei<<'\n';
        cout<<suzume<<'\n';
        cout<<suzume - min(lei, rei) + 1<<'\n';
    }
}
// Se vuelve más fácil,
