...
signed main()
{
    ...
    while(t--)
    {
        ...
        cin>>n;
        vi ar(n);
        int maximus_decimus_meridius = 0;
        int residencia_salguero = 0;
        fore(i, 0, n)
        {
            cin>>ar[i];
            if(i & 1)
            {
                residencia_salguero = max(residencia_salguero,
                maximus_decimus_meridius + abs(ar[i - 1]));
                int absolut = max(ar[i] + ar[i - 1], -ar[i] - ar[i - 1]);
                maximus_decimus_meridius = max({maximus_decimus_meridius + absolut,
                                abs(ar[i]), absolut});
            }
            residencia_salguero = max(residencia_salguero, maximus_decimus_meridius);
        }
        cout<<residencia_salguero<<'\n';
    }
}
// Se vuelve más fácil,
