...
signed main()
{
    ...
    while(t--)
    {
        ...
        fore(i, 0, cana + 2)
        {
            ...
            if(visi[i])
            {
                mayonesa = max(mayonesa, i);
                if(visi[i] > 1)
                    for(int x : salto[i])
                        visi[x]++;
            }
        }
        for(int i = cana + 1; i > -1; i--)
            if(mevoy[i] != 0)
                mevoy[i] = max(mevoy[i], mevoy[mevoy[i]]);
        int res = 0;
        fore(i, 0, min(m + 1, cana + 2))
        {
            res += max({mayonesa, mevoy[i], i});
            // cout<<i<<' '<<mayonesa<<' '<<mevoy[i]<<'\n';
        }
        // cout<<cana + 2<<'\n';
        if(m >= cana + 2)
        {
            cana++;
            res += (m + cana + 1) * (m - cana) / 2;
        }
        cout<<res<<'\n';
    }
	return 0;
