...
signed main()
{
    ...
    sort(all(ar), [](ii a, ii b){return mp(a.f, -a.s)< mp(b.f, -b.s);});
    fore(i, 0, n)
        bea[i].f = ar[i].s, bea[i].s = i;
    sort(all(bea));
    reverse(all(bea));
    ...
    fore(i, 1, n + 1)
    {
        dp[i] = dp[i - 1];
        fore(j, ar[i - 1].f, w + 1)
            dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - ar[i - 1].f] + ar[i - 1].s);
        vi fantom(n);
        int rea = dp[i][w];
        int popo = w;
        for(int j = i; j > 0; j--)
        {
            if(dp[j][popo] > dp[j - 1][popo])
            {
                fantom[j - 1] = 1;
                popo -= ar[j - 1].f;
            }
