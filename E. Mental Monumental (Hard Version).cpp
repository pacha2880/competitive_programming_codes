...
signed main()
{
    ...
    while(t--){
        ...
        vi are(n);
        int mama = 0;
        fore(i, 0, n) cin>>are[i], mama = max(mama, are[i]);
        mama+=2;
        init(0, mama - 1, 0);
        update(0, mama - 1, 0, 0, 0, -1);
        int resi = 0;
        vi coso(mama);
        fore(i, 0, n){
            int x = are[i];
            if(coso[x] || x > resi){
                update(0, mama - 1, 0, 0, (x - 1) / 2, 1);
            }
            else{
                update(0, mama - 1, 0, x, x, MOD);
                update(0, mama - 1, 0, 0, x, 1);
            }
            coso[x] = 1;
            while(query(0, mama - 1, 0, 0, resi) >= 0){
                resi++;
                update(0, mama - 1, 0, 0, resi, -1);
                if(coso[resi]){
                    update(0, mama - 1, 0, 0, (resi - 1) / 2, -1);
                    update(0, mama - 1, 0, resi, resi, MOD);
                    update(0, mama - 1, 0, 0, resi, 1);
                }
            }
            // fore(i, 0, resi + 1) cout<<query(0, mama - 1, 0, i, i)<<' ';
            // cout<<'\n';
            if(i > 0) cout<<' ';
            cout<<resi;
            // cout<<'\n';
