...
node join(node a, node b)
{
	node n;
	n.ar.clear();
	for(int x : a.ar) n.ar.pb(x);
	for(int x : b.ar) n.ar.pb(x);
	sort(all(n.ar));
	while(n.ar.size() > 31) n.ar.pop_back();
  	return n;
}
int ar[tam];
node t[4 * tam];
...
int main()
{
	...
	while(t--)
	{
		...
		while(q--)
		{
			...
			int res = (1<<30) - 1;
			fore(i, 0, x.ar.size())
			fore(j, i + 1, x.ar.size())
				res = min(res, x.ar[i] | x.ar[j]);
			cout<<res<<'\n';
		}
	}
	return 0;
