...
signed main()
{
    ...
    while(t--)
    {
        ...
        fore(i, 0, n)
        {
            ...
            map<int, vi> suzume;
            int acu = 0;
            auto add = [&](int pos, int bit, int cant){
                if(!suzume.count(pos))
                    suzume[pos] = vi(k);
                suzume[pos][bit] += cant;
            };
            ...
            {
                if(ar[i] & (1<<j))
                    ultimi[j] = i, ceros[j] = 0;
                else
                    ceros[j]++;
                // cout<<'-'<<j<<' '<<ceros[j]<<' '<<ultimi[j]<<'\n';
                if(ceros[j] > 0)
                {
                    if(ceros[j] % 2 == 0)
                    {
            ...
            for(auto &cat : suzume)
            {
                ...
                pres[cat.f].pb(nu), cucu[i].pb(nu);
                // cout<<"su "<<cat.f<<' '<<nu<<'\n';
            }
            
        }
        vi res(n);
        fore(i, 0, n) res[i] = ar[i];
        multiset<int> ansu;
        // cout<<((~(ar[1] | ar[2])) & ((1<<17) - 1)) <<'\n';;
        fore(i, 0, n)
        {
            for(int x : pres[i]) ansu.insert(x);
            if(!ansu.empty())
                res[i] = max(res[i], *--ansu.end());
            for(int x : cucu[i]) ansu.erase(ansu.find(x));
        }
