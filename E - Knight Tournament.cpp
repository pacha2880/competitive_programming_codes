...
int ar[tam], t[tam * 4];
void pelea(int b, int e, int node, int i, int j, int gana)
{
	if(e < i || b > j) return;
	if(i <= b && j >= e && t[node] == e - b + 1) return;
	if(b == e)
	{
		if(b + 1 != gana)	
			t[node] = 1, ar[b] = gana;
	...
	pelea(b, mid, l, i, j, gana);
	pelea(mid + 1, e, r, i, j, gana);
	t[node] = t[l] + t[r];
}
